...
# 2. リポジトリ名の初期値設定（「ユーザー名/リポジトリ名」をvalueに書いておくと毎回入力の手間が省けます）
REPO_NAME = st.sidebar.text_input(
    "Repository", 
    value="rioki3/Surveycad",  # 👈ここを書き換えておくと便利です
    ...
    with zipfile.ZipFile(zip_buffer, "r") as z:
        for file_path in z.namelist():
            ...
            with z.open(file_path) as f:
                raw_data = f.read()
                try:
                    # 【文字コード修正】まずは標準の UTF-8 でデコードを試みる
                    file_content = raw_data.decode("utf-8")
                except UnicodeDecodeError:
                    try:
                        # 失敗した場合、日本語環境で多い Shift-JIS (cp932) で試みる
                        file_content = raw_data.decode("cp932")
                    except UnicodeDecodeError:
                        st.warning(
                            f"⚠️ テキスト（UTF-8/Shift-JIS）ではないためスキップ: {file_path}"
                        )
                        continue
